...
<html lang="en">
  ...
  <body class="auctionHouseBody">
    <% cs = conn.prepareCall("call ProduceMailingList()");
      cs.execute();
      // TODO. Error handling
      cs.getMoreResults();
      mailingListRes = cs.getResultSet();
    %>
    <div class="container">
      ...
      <table id ="bestSellersTable" class="table table-striped table-bordered dt-responsive nowrap auctionHouseTable">
        ...
        <tbody>
          <% while(mailingListRes.next()) { %>
          <tr>
            <td>
              <%= mailingListRes.getString(3)%>
            </td>
            <td>
              <%= mailingListRes.getString(2)%>
            </td>
            <td>
              <%= mailingListRes.getString(1)%>
            </td>
          </tr>
          <% } %>
        </tbody>
