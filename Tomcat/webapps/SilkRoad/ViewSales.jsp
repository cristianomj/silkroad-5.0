...
try	{
 ...
 switch (searchType) {
   case "all":
   cs = conn.prepareCall("call GetAllSales()");
   cs.execute();
   revenueListRes = cs.getResultSet();
   break;

   case "seller":
   cs = conn.prepareCall("call GetSalesBySeller(?)");
   ...
   case "itemName":
   cs = conn.prepareCall("call GetSalesByItemName(?)");
   cs.setString(1, searchValue);
   cs.execute();
   revenueListRes = cs.getResultSet();
   break;
           ...
           <ul class="nav">
            ...
            <script>
              function ViewSalesRefresh(selection) {
               ...
               window.location = "ViewSales.jsp?searchType="+searchType+"&searchValue="+searchValue;
              }
            </script>
            <%-- <script>
            $(function() {
            ...
            <body class="auctionHouseBody">
             ...
             <table id ="bestSellersTable" class="table table-striped table-bordered dt-responsive nowrap auctionHouseTable">
              ...
              <tbody>
               ...
               <tr>
                <td>
                 <%= revenueListRes.getString("Buyer")%>
                </td>
                <td>
                 <%= revenueListRes.getString("Seller") %>
                </td>
                <td>
                 <%= revenueListRes.getDouble("Price")%>
                </td>
                <td>
                 <%= revenueListRes.getString("Date")%>
                </td>
                <td>
                 <%= revenueListRes.getString("ItemName")%>
                </td>
                <td>
                 <%= revenueListRes.getString("ItemType")%>
                </td>
                <td>
                 <%= revenueListRes.getInt("AuctionID")%>
                </td>
               </tr>
               <% } %>
              </tbody>
